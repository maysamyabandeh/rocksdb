...
void Compaction::MarkFilesBeingCompacted(bool mark_as_compacted) {
  ...
  ROCKS_LOG_DEBUG(immutable_cf_options_.info_log, "MARK %d", mark_as_compacted);
  for (size_t i = 0; i < num_input_levels(); i++) {
    for (size_t j = 0; j < inputs_[i].size(); j++) {
     ROCKS_LOG_DEBUG(immutable_cf_options_.info_log, "i=%d j=%lu", inputs_[i].level, inputs_[i][j]->fd.packed_number_and_path_id);
...
const char* Compaction::InputLevelSummary(
  ...
  for (auto& input_level : inputs_) {
    ...
    if (!is_first) {
      ...
      len = std::min((size_t)len, sizeof(scratch->buffer));
    } else {
      is_first = false;
    }
